...
.gitignore に含まれるファイルはサブフォルダ内のファイルも含めて表示上で薄くなり，実際のデプロイ対象からも除外されます．  
...
### 5. リモートに存在する不足ファイルの扱い

選択したローカルフォルダ内に，リモートリポジトリ側の既存ファイルが存在しない場合の扱いを選択できます．

#### ①残して続行

ローカルに存在しない追跡済みファイルを削除扱いにせず，リモートリポジトリ側に残します．  
既存リポジトリの一部だけを更新したい場合はこちらを使用してください．

#### ②削除を反映

ローカルに存在しない追跡済みファイルを削除としてコミットします．  
ローカルフォルダの内容でリポジトリ全体を同期したい場合に使用します．

### 6. gitignore対象ファイルの削除

`gitignore対象を削除` ボタンを実行すると，選択中リポジトリ内で既に追跡されている `.gitignore` 対象ファイルだけを抽出し，リポジトリから削除するコミットを作成します．  
ローカルファイルは削除せず，Git の追跡対象からだけ外します．
